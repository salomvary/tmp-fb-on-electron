...
__d('Typeahead', ['ArbiterMixin', 'BehaviorsMixin', 'DataStore', 'DOM', 'Event', 'Parent', 'Run', 'Style', 'emptyFunction', 'ge', 'mixin'], (function a(b, c, d, e, f, g) {
    ...
    j.prototype.getView = function() {
        ...
        if (!this.view) {
            ...
            if (typeof k.ctor === 'string') {
                this.view = new window[k.ctor](l, k.options || {});
            } else this.view = new k.ctor(l, k.options || {});
            this.view.init();
            this.view.setTypeahead(this.element);
        }
        return this.view;
    };
    j.prototype.getCore = function() {
        'use strict';
        if (!this.core) {
            var k = this.args.core;
            if (typeof k.ctor === 'string') {
                this.core = new window[k.ctor](k.options || {});
            } else this.core = new k.ctor(k.options || {});
    ...
    j.prototype.setHeight = function(k) {
        'use strict';
        if (k !== 'auto') k += 'px';
        c('Style').set(this.element, 'height', k);
    };
    j.prototype.swapData = function(k) {
        'use strict';
        return this.$Typeahead1(k, true);
    };
    j.prototype.swapDataNoCoreReset = function(k) {
        'use strict';
        return this.$Typeahead1(k, false);
    };
    j.prototype.$Typeahead1 = function(k, l) {
        'use strict';
        var m = this.core;
    ...
    j.prototype.proxyEvents = function() {
        'use strict';
        [this.data, this.view, this.core].forEach(function(k) {
            k.subscribe(k.events, this.inform.bind(this));
        }, this);
    };
    j.prototype.initBehaviors = function(k) {
        'use strict';
        k.forEach(function(l) {
            if (typeof l === 'string') {
                if (b.TypeaheadBehaviors && b.TypeaheadBehaviors[l]) {
                    b.TypeaheadBehaviors[l](this);
                } else c('Run').onLoad(function() {
                    if (b.TypeaheadBehaviors)(b.TypeaheadBehaviors[l] || c('emptyFunction'))(this);
                }.bind(this));
            } else this.enableBehavior(l);
        }, this);
    };
    ...
    j.initNow = function(k, l, m) {
        'use strict';
        if (m) m.init(k);
        k.init(l);
    };
    j.init = function(k, l, m, n) {
        var o;
        'use strict';
        if (!c('DOM').isNodeOfType(k, ['input', 'textarea'])) k = c('DOM').scry(k, 'input')[0] || c('DOM').scry(k, 'textarea')[0];
        var p = false;
        try {
            p = document.activeElement === k;
        } catch (q) {}
        if (p) {
            j.initNow(l, m, n);
        } else(function() {
...
__d('BasicTypeaheadRenderer', ['BadgeHelper', 'DOM'], (function a(b, c, d, e, f, g) {
    ...
    function h(i, j) {
        var k = [];
        if (i.icon) k.push(c('DOM').create('img', {
            alt: '',
            src: i.icon
        }));
        var l = i.debug_info;
        if (l) k.push(c('DOM').create('span', {
            className: 'debugInfo'
        }, l));
        if (i.text) {
            var m = [i.text];
            if (i.is_verified) m.push(c('BadgeHelper').renderBadge('xsmall', 'verified'));
...
__d('createIxElement', ['invariant', 'DOM', 'joinClasses'], (function a(b, c, d, e, f, g, h) {
    ...
    function i(j, k) {
        var l = 'img',
            m;
        !(j.sprited || j.uri) ? h(0): void 0;
        if (j.sprited) {
            l = c('joinClasses')(l, j.spriteMapCssClass, j.spriteCssClass);
            m = c('DOM').create('i', {
                className: l
            });
            if (k != null) c('DOM').setContent(m, c('DOM').create('u', null, k));
        } else if (j.uri) {
            m = c('DOM').create('img', {
                className: l,
                src: j.uri
            });
            if (k != null) m.setAttribute('alt', k);
            if (j.width) m.setAttribute('width', j.width);
            if (j.height) m.setAttribute('height', j.height);
        }
        return m;
    }
    f.exports = i;
}), null);
__d('TypeaheadView', ['ix', '$', 'ArbiterMixin', 'BasicTypeaheadRenderer', 'CSS', 'DOM', 'Event', 'Parent', 'createIxElement', 'emptyFunction', 'getElementText', 'getOrCreateDOMID', 'mixin'], (function a(b, c, d, e, f, g, h) {
    var i, j;
    if (c.__markCompiled) c.__markCompiled();
    ...
    k.prototype.setTypeahead = function(l) {
        'use strict';
        this.typeahead = l;
    };
    k.prototype.setAccessibilityControlElement = function(l) {
        'use strict';
    ...
    k.prototype.mouseup = function(event) {
        'use strict';
        if (event.button != 2) {
            this.select(true);
            event.prevent();
    ...
    k.prototype.reset = function(l) {
        'use strict';
        if (!l) this.disableAutoSelect = false;
        this.index = -1;
        this.items = [];
        this.results = [];
        this.value = '';
    ...
    k.prototype.getIndex = function(event) {
        'use strict';
    ...
    k.prototype.show = function() {
        ...
        if (this.results && this.results.length)
            if (this.autoSelect && this.accessibilityElement && this.selected) this.accessibilityElement.setAttribute('aria-activedescendant', c('getOrCreateDOMID')(this.selected));
    ...
    k.prototype.render = function(l, m, n) {
        ...
        m = o.results;
        var p = n && this.index != -1 ? this.index : this.getDefaultIndex(m);
        this.results = m;
    ...
    k.prototype.buildResults = function(l) {
        ...
        var o = l.map(function(q, r) {
            ...
            p = c('DOM').create('ul', {
                className: n,
                id: 'typeahead_list_' + (this.typeahead ? c('getOrCreateDOMID')(this.typeahead) : c('getOrCreateDOMID')(this.element))
            }, o);
        p.setAttribute('role', 'listbox');
        return p;
    };
    k.prototype.showLoadingIndicator = function() {
        'use strict';
        var l = c('createIxElement')(h('/images/loaders/indicator_blue_small.gif')),
            m = c('DOM').create('li', {
    ...
    k.prototype.getDefaultIndex = function() {
        'use strict';
    ...
    k.prototype.getItemText = function(l) {
        'use strict';
        var m = '';
        if (l) {
    ...
    k.prototype.setIsViewingSelectedItems = function(l) {
        'use strict';
        this.viewingSelected = l;
        return this;
    };
    k.prototype.getIsViewingSelectedItems = function() {
        'use strict';
    ...
    k.prototype.highlight = function(l, m) {
        ...
        if (l > this.items.length - 1) {
            l = -1;
        } else if (l < -1) l = this.items.length - 1;
        if (l >= 0 && l < this.items.length) {
            ...
            if (this.accessibilityElement) setTimeout(function() {
                this.accessibilityElement.setAttribute('aria-activedescendant', c('getOrCreateDOMID')(this.selected));
            }.bind(this), 0);
        } else this.accessibilityElement && this.accessibilityElement.removeAttribute('aria-activedescendant');
        this.index = l;
        this.disableAutoSelect = l == -1;
        if (m !== false) this.inform('highlight', {
            index: l,
            selected: this.results[l],
            element: this.selected
        });
    };
    k.prototype.select = function(l) {
        'use strict';
        if (this.headerIndex && l) return;
        var m = this.index,
            n = this.results[m],
        ...
        if (n) {
            ...
            if (this.shouldValidateTypeaheadSelection(n)) {
                this.validateTypeaheadSelection(n, p);
            } else p(n);
        }
    };
    k.prototype.shouldValidateTypeaheadSelection = function(l) {
        'use strict';
        return false;
    };
    k.prototype.validateTypeaheadSelection = function(l, m) {
        'use strict';
    };
...
__d('URLMatcher', ['URLMatcherConfig'], (function a(b, c, d, e, f, g) {
    ...
    var h = '!"#%&\'()*,-./:;<>?@[\\]^_`{|}',
        ...
        t = '(?:' + '(?:' + k + p + q + ')|' + '(?:' + l + q + ')|' + '(?:' + m + q + ')|' + '(?:' + n + p + s + q + ')|' + '(?:' + p + s + q + r + ')' + ')',
        u = '[/#?]',
        v = '\\([^\\s()<>]+\\)',
        w = '[^\\s()<>?#]+',
        x = new RegExp(t, 'im'),
        ...
        z = new RegExp(y, 'im'),
        aa = '(?:' + '(?:' + u + ')' + '(?:' + '(?:' + v + '|' + w + ')*' + ')*' + ')*',
        ba = new RegExp('(' + '(?:' + t + ')' + '(?:' + aa + ')' + ')', 'im'),
        ca = new RegExp('(' + '(?:' + k + p + q + ')|' + '(?:' + n + p + s + q + ')' + ')'),
        da = /[\s'";]/,
        ea = new RegExp(u, 'im'),
        fa = new RegExp('[\\s!"#%&\'()*,./:;<>?@[\\]^`{|}\u00ab\u00bb\u2000-\u206F\uff08\uff09]', 'im'),
        ga = new RegExp('[\\s()<>?#]', 'im'),
        ...
        na = function pa(qa) {
            do {
                ...
                if (!ra) return null;
                var sa = false;
                if (ra[0][0] === '[' && ra.index > 0 && qa[ra.index - 1] === '@') {
                    var ta = z.exec(ra[0]);
                    if (ta) {
                        sa = true;
                        qa = qa.substr(ra.index + ta[0].length);
                    }
                }
            } while (sa);
            var ua = qa.substr(ra.index + ra[0].length);
            if (ua.length === 0 || !ea.test(ua[0])) return ia(ra[0]);
            var va = 0,
                wa = 0,
                xa = 1,
                ya = 0,
            ...
            for (var ab = 1; ab < ua.length; ab++) {
                ...
                if (za === wa) {
                    if (bb === '(') {
                        ya = ya + 1;
                        za = xa;
                    } else if (ea.test(bb) || !fa.test(bb)) {
                        va = ab;
                    } else if (ga.test(bb)) break;
                } else if (bb === '(') {
                    ya = ya + 1;
                } else if (bb === ')') {
                    ya = ya - 1;
                ...
                } else if (ha.test(bb)) break;
            }
            return ia(ra[0] + ua.substring(0, va + 1));
        },
        oa = {};
...
__d('StaticContainer.react', ['React'], (function a(b, c, d, e, f, g) {
    ...
    i = h && h.prototype;
    j.prototype.shouldComponentUpdate = function(k) {
        'use strict';
