if (self.CavalryLogger) {
    CavalryLogger.start_js(["Rpr16"]);
}

__d('HistoryManager', ['SessionName', 'Cookie', 'Env', 'Event', 'URI', 'UserAgent_DEPRECATED', 'isFacebookURI', 'emptyFunction', 'goOrReplace', 'isInIframe', 'setIntervalAcrossTransitions'], (function a(b, c, d, e, f, g) {
    ...
    var h = {
        ...
        init: function i() {
            ...
            if ('onhashchange' in window) {
                c('Event').listen(window, 'hashchange', function() {
                    setTimeout(h.checkURI.bind(h), 0);
                });
            } else c('setIntervalAcrossTransitions')(h.checkURI, 42);
            return h;
        },
        ...
        notify: function i(j) {
            ...
            for (var k = 0; k < h.callbacks.length; k++) try {
                if (h.callbacks[k](j)) return true;
            } catch (l) {}
            return false;
        },
        checkURI: function i() {
        ...
        _updateRefererURI: function i(j) {
            j = j.toString();
            if (j.charAt(0) != '/' && j.indexOf('//') == -1) return;
            var k = new(c('URI'))(window.location);
            if (c('isFacebookURI')(k)) {
                var l = k.getPath() + window.location.search;
            } else var l = '';
            var m = new(c('URI'))(j).getQualifiedURI().setFragment(l).toString(),
                n = 2048;
            if (m.length > n) m = m.substring(0, n) + '...';
            c('Cookie').set('x-referer', m);
        },
        go: function i(j, k, l) {
            if (window.history && history.pushState) {
                k || typeof j == 'number';
                var m = new(c('URI'))(j).removeQueryData('ref').toString();
                h.lastChanged = Date.now();
                this.lastURI = m;
                if (l) {
                    window.history.replaceState(j, null, m);
                } else window.history.pushState(j, null, m);
                if (c('UserAgent_DEPRECATED').webkit() < 534) h._updateRefererURI(j);
                return false;
            }
            h.debug('go: ' + j);
        ...
        getCurrentFragment: function i() {
            ...
            return j == h.orig_fragment ? h.canonical.getFragment() : j;
        }
    };
    f.exports = h;
}), null);
__d('escapeJSQuotes', [], (function a(b, c, d, e, f, g) {
    ...
    function h(i) {
        if (typeof i == 'undefined' || i == null || !i.valueOf()) return '';
        return i.toString().replace(/\\/g, '\\\\').replace(/\n/g, '\\n').replace(/\r/g, '\\r').replace(/"/g, '\\x22').replace(/'/g, '\\\'').replace(/</g, '\\x3c').replace(/>/g, '\\x3e').replace(/&/g, '\\x26');
    }
    f.exports = h;
}), null);
    ...
    var p = {
        ...
        _init: function q() {
            ...
            c('Event').listen(window, 'scroll', function() {
                if (!p._scroll_locked) l(p._current_uri, c('Vector').getScrollPosition());
            });
            return p;
        },
        ...
        getCurrentURI: function q(r) {
            this._init();
            if (!p._current_uri && !r) return new(c('URI'))(p._most_recent_uri);
        ...
        go: function q(r, s) {
            ...
            c('JSLogger').create('pagetransition').debug('go', {
                uri: u.toString()
            });
            n(u);
            ...
            p._loadPage(u, function(v) {
                if (v) {
                    c('ModalLayer').unfixed(function() {
                        c('HistoryManager').go(u.toString(), false, s);
                    });
                } else c('goOrReplace')(window.location, u, s);
            });
        },
        _historyManagerHandler: function q(r) {
            ...
            if (!c('ScriptPath').getClickPointInfo()) c('ScriptPath').setClickPointInfo({
                click: 'back'
            });
            p._loadPage(new(c('URI'))(r), function(s) {
                if (!s) c('goOrReplace')(window.location, r, true);
            });
            return true;
        },
        _loadPage: function q(r, s) {
            ...
            p._next_uri = v;
            if (w) {
                p._warnBeforeLeaving(w, u);
            } else u();
        },
        _handleTransition: function q(r) {
            window.onbeforeleavehooks = undefined;
            if (p._transitions_disabled || !r.isSameOrigin()) return false;
            ...
            for (var w = v.length - 1; w >= 0; --w) {
                ...
                for (var y = x.length - 1; y >= 0; --y) {
                    ...
                    if (z === true || z === c('PageTransitionsRegistrar').DELAY_HISTORY) {
                        ...
                        try {
                            c('Arbiter').inform("page_transition", aa);
                        } catch (ba) {}
                        return z;
                    } else x.splice(y, 1);
                }
        ...
        rewriteCurrentURI: function q(r, s) {
            ...
            c('PageTransitionsRegistrar').registerHandler(function() {
                if (r == p.getMostRecentURI().getUnqualifiedURI().toString()) {
            ...
            p.go(s, true);
            !(t.length === u + 1 && t[u].length === (v ? 0 : 1)) ? h(0): void 0;
            t.length = u;
        },
        _warnBeforeLeaving: function q(r, s) {
            c('Bootloader').loadModules(["DialogX", "XUIDialogTitle.react", "XUIDialogBody.react", "XUIDialogButton.react", "XUIDialogFooter.react", "XUIGrayText.react"], function(t, u, v, w, x, y) {
                ...
                }, c('React').createElement('div', null, c('React').createElement(u, {
                    showCloseButton: false
                }, i._("Leave Page?")), c('React').createElement(v, null, c('React').createElement(y, {
                    shade: 'medium',
                    size: 'medium'
                }, r)), c('React').createElement(x, null, c('React').createElement(w, {
                    action: 'cancel',
...
__d("XEventCreateDialogController", ["XController"], (function a(b, c, d, e, f, g) {
    c.__markCompiled && c.__markCompiled();
    f.exports = c("XController").create("\/events\/dialog\/create\/", {
        acontext: {
            type: "String",
            required: true
        },
